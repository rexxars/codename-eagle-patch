...
| `menu/menufont.tga`     | The menu font (`menufont.tga`, 80×144, inside `menu/menupics.dat`) with real glyphs painted into the punctuation cells the stock font left as placeholder dots — the full set the player-name sanitiser allows (`" $ % & ' * + / : ; < = > @ [ \ ] { \| }`), in both the white and dim colour halves. Every other pixel is byte-identical to the stock font. `textool set --allow-any` replaces it inside the player's `menu/menupics.dat` at install time (`--allow-any` because menu bitmaps are not square/power-of-two like the in-game textures). |
...
| `src/menufont.png`      | The 80×144 human-oriented image `menufont.tga` encodes (provenance/QC): the stock menu font with the added punctuation glyphs. Same orientation as the TGA (engine row order = top-down), so no flip.                                                                                                                                                                                                                                                                                                                                                  |
